...
Perceived Stress (Continuous): (not stressed)1-5(very stressed)
Emotional Valence(Continuous): (negative emotion)1-9(postive emotions)
Condition (Categorical): condition 1: non stigamtized, 2= stigmatized
...
```{r}
numeric_condition <- as.numeric(data$Condition)

if (any(is.na(numeric_condition)))
  stop("There are missing values in data$Condition. Please handle them before computing SD.")

c(summary(numeric_condition), "SD" = sd(numeric_condition))
```
...
```
## Results
The interaction between stigma, percieved stress, and emotional valence was significant. This supports the idea that stigma plays a moderating role in the link between perceived stress and emotional valence. 
